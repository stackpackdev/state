...
export function createFlow(options) {
    ...
    if (!states.includes(initial)) {
        throw new Error(`Flow "${name}": initial state "${initial}" is not in valid states [${states.join(', ')}]`);
    }
        ...
        go(state, actor) {
            ...
            if (!states.includes(state)) {
                if (process.env.NODE_ENV === 'development') {
                    console.error(`Flow "${name}": invalid state "${state}". Valid states: [${states.join(', ')}]`);
                }
                return;
            }
    ...
    function recordTransition(from, to, actor) {
        history = [
            { from, to, actor, timestamp: Date.now() },
            ...history,
        ].slice(0, 1000);
    }
    function notifyListeners(current, prev, actor) {
