 ...
 */
export function extractModes(schema) {
 ...
 */
export function createModeError(storeName, currentMode, attemptedState, discriminant) {
    const attemptedMode = attemptedState && typeof attemptedState === 'object'
    ...
    return (`[state-agent] Store "${storeName}": mode transition ${modeInfo}. ` +
 ...
 */
export function deriveGatesFromModes(modeInfo) {
    const gates = {};
    for (const modeName of modeInfo.modeNames) {
        gates[modeName] = (state) => state && state[modeInfo.discriminant] === modeName;
    }
    return gates;
}
/**
 ...
 */
export function deriveWhenFromModes(modeInfo) {
    const when = {};
    for (const modeName of modeInfo.modeNames) {
        const capitalizedName = modeName.charAt(0).toUpperCase() + modeName.slice(1);
        when[`is${capitalizedName}`] = (state) => state && state[modeInfo.discriminant] === modeName;
    }
    return when;
}
